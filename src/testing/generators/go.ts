...
function parseGoParams(paramsStr: string): GoParam[] {
  ...
  let lastType = '';
  const parsed: Array<{ names: string[]; type: string }> = [];
...
function generateTestCode(packageName: string, functions: GoFunction[]): string {
...
function getZeroValue(goType: string): string {
  switch (goType) {
    case 'int':
    case 'int8':
    case 'int16':
