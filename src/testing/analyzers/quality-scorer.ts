import type { TestQualityScore, TestQualityMetrics } from './types.js';
...
export async function scoreTestQuality(options: ScoreTestQualityOptions): Promise<TestQualityScore> {
  ...
  const hasSharedState =
    testCode.includes('let ') && !testCode.match(/test\([^)]*\)\s*=>\s*\{[^}]*let /s) ||
