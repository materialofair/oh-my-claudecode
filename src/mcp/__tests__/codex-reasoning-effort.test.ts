...
import type { ReasoningEffort } from '../codex-core.js';
...
describe('executeCodex reasoning effort', () => {
  ...
  function setupSpawnMock() {
    ...
    const childOn = vi.fn((event: string, cb: (...args: any[]) => void) => {
      if (event === 'close') {
  ...
  it('should NOT include -c flag for invalid reasoning effort value', async () => {
    setupSpawnMock();
    const promise = executeCodex('test prompt', 'gpt-5.3-codex', undefined, 'invalid' as ReasoningEffort);
    await promise;
...
describe('executeCodexWithFallback reasoning effort passthrough', () => {
  it('should pass reasoningEffort to the executor function', async () => {
    ...
    await executeCodexWithFallback(
      'test prompt',
  ...
  it('should preserve reasoningEffort across fallback chain retries', async () => {
    ...
    const result = await executeCodexWithFallback(
      'test prompt',
      undefined, // no explicit model -> use fallback chain
      undefined,
