 ...
 */
function getGitDiff(directory: string, gitStartCommit: string): string {
  try {
    const diff = execSync(
      `git diff ${gitStartCommit}..HEAD`,
 ...
 */
function getChangedFiles(directory: string, gitStartCommit: string): string[] {
  try {
    const output = execSync(
      `git diff --name-only ${gitStartCommit}..HEAD`,
 ...
 */
function getCommitMessages(directory: string, gitStartCommit: string): string[] {
  try {
    const output = execSync(
      `git log ${gitStartCommit}..HEAD --pretty=format:"%h - %s (%an, %ar)"`,
      { cwd: directory, encoding: 'utf-8' }
    );
    return output.trim().split('\n').filter(line => line.length > 0);
  } catch (error: any) {
...
export async function reviewTrack(
  ...
  slug: string,
  sessionId?: string
): Promise<{ success: boolean; reviewPath?: string; error?: string }> {
  try {
    // Load track
