...
import type { Track, TrackTask, ConductorContext } from './types.js';
import {
  readConductorState,
  createTrack as createTrackState,
  updateTrack,
  getSpecPath,
  getPlanPath,
  ensureConductorDirs
 ...
 */
function getCurrentGitCommit(directory: string): string | null {
  try {
 ...
 */
function createGitBranch(directory: string, slug: string): string | null {
  try {
 ...
 * 3. Yet another task
 */
function parsePlanTasks(planContent: string, trackSlug: string): TrackTask[] {
 ...
 */
async function delegateToAnalyst(
 ...
 * Returns the Task tool invocation string
 */
async function delegateToArchitect(
  context: ConductorContext,
  specPath: string,
  planPath: string
): Promise<string> {
...
): Promise<{ track: Track; instruction: string }> {
 ...
 */
export async function proceedToPlanning(
  directory: string,
  slug: string,
  sessionId?: string
): Promise<{ track: Track; instruction: string }> {
  const state = readConductorState(directory, sessionId);
  if (!state) throw new Error('Conductor not initialized');

  const track = state.tracks[slug];
  if (!track) throw new Error(`Track ${slug} not found`);
...
export async function reviewTrack(
  ...
  try { specContent = readFileSync(specPath, 'utf-8'); } catch {}
  try { planContent = readFileSync(planPath, 'utf-8'); } catch {}
 ...
 */
export function getSpecContent(directory: string, slug: string): string | null {
  const specPath = getSpecPath(directory, slug);
  if (!existsSync(specPath)) {
    return null;
  }
 ...
 */
export function getPlanContent(directory: string, slug: string): string | null {
  const planPath = getPlanPath(directory, slug);
