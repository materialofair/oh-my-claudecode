 ...
 */
function isWorkingDirectoryClean(directory: string): boolean {
  try {
    const output = execSync(
...
function executeGitReset(
  ...
  try {
    execSync(
      `git reset ${resetMode} ${gitStartCommit}`,
      { cwd: directory, encoding: 'utf-8' }
    );
  } catch (error: any) {
 ...
 */
function deleteBranch(directory: string, branch: string, force: boolean = true): void {
 ...
 */
function formatRevertPreview(track: Track, changedFiles: string[]): string {
  const lines: string[] = [];
...
export async function revertTrack(
  ...
  const {
    ...
    deleteBranch: shouldDeleteBranch = true
  } = options || {};
