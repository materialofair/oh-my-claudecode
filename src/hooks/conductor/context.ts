...
import type { ProjectMemory } from '../project-memory/types.js';
import type { ConductorContext } from './types.js';

/**
 * Path to conductor context directory within .omc
 */
function getContextDir(directory: string): string {
 ...
 */
export function bootstrapFromProjectMemory(directory: string): string {
  ...
  const projectMemory: ProjectMemory = JSON.parse(
    readFileSync(projectMemoryPath, 'utf-8')
  );
  ...
  if (techStack.frameworks.length > 0) {
    ...
    const byCategory: Record<string, typeof techStack.frameworks> = {};
 ...
 */
export function generateProductMd(directory: string, description: string): string {
 ...
 */
export function generateWorkflowMd(directory: string, guidelines?: string): string {
 ...
 */
export function loadContext(directory: string): string {
 ...
 */
export function isContextInitialized(directory: string): boolean {
  const contextDir = getContextDir(directory);
