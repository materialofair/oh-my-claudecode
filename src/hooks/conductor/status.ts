...
import type { Track, TrackStatus, ConductorState } from './types.js';
import { readConductorState, getAllTracks } from './state.js';

/**
 * Get status indicator character for a track status
 */
function getStatusIndicator(status: TrackStatus): string {
  switch (status) {
    case 'done':
      return '[x]';
    case 'in_progress':
 ...
 */
function formatTrackSummary(track: Track, isActive: boolean): string {
 ...
 */
function formatTrackDetails(track: Track): string {
  const lines: string[] = [];
...
export function displayStatus(
  ...
  const byStatus: Record<TrackStatus, Track[]> = {
    spec: [],
    planned: [],
    in_progress: [],
    review: [],
  ...
  const statusOrder: TrackStatus[] = ['in_progress', 'review', 'planned', 'spec', 'done', 'reverted'];
