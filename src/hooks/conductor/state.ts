...
import type { ConductorState, ConductorContext, Track, TrackTask, TrackStatus, ConductorPhase } from './types.js';
 ...
 */
export function ensureConductorDirs(directory: string): void {
  const dirs = [
 ...
 */
export function readConductorState(directory: string, sessionId?: string): ConductorState | null {
  if (sessionId) {
 ...
 */
export function writeConductorState(directory: string, state: ConductorState, sessionId?: string): boolean {
  try {
 ...
 */
export function clearConductorState(directory: string, sessionId?: string): boolean {
  const stateFile = getStateFilePath(directory, sessionId);
...
export function updateContext(
  directory: string,
  updates: Partial<ConductorContext>,
  sessionId?: string
): boolean {
  const state = readConductorState(directory, sessionId);
  if (!state) {
 ...
 */
export function updateTrackTask(
  directory: string,
  slug: string,
  taskId: string,
  updates: Partial<TrackTask>,
  sessionId?: string
): boolean {
  const state = readConductorState(directory, sessionId);
  if (!state || !state.tracks[slug]) {
 ...
 * Get all tracks
 */
export function getAllTracks(directory: string, sessionId?: string): Track[] {
  const state = readConductorState(directory, sessionId);
  if (!state) {
    return [];
  }
 ...
 */
export function isConductorActive(directory: string, sessionId?: string): boolean {
  const state = readConductorState(directory, sessionId);
 ...
 */
export function deactivateConductor(directory: string, sessionId?: string): boolean {
  const state = readConductorState(directory, sessionId);
  if (!state) {
    return false;
  }
 ...
 */
export function getSpecPath(directory: string, slug: string): string {
 ...
 */
export function getPlanPath(directory: string, slug: string): string {
 ...
 */
export function getReviewPath(directory: string, slug: string): string {
