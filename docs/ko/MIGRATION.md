...
- **npm 패키지명**: `claudecode-omc`

> **왜 이름이 다른가요?** 배포용 npm 패키지명은 `claudecode-omc`이고, 프로젝트, GitHub 저장소, 플러그인 및 모든 명령어는 `oh-my-claudecode`를 사용합니다.
...
```bash
npm uninstall -g claudecode-omc
```
...
- 패키지명: `claudecode-omc` → `oh-my-claudecode`
...
- 공지를 위해 [GitHub 저장소](https://github.com/materialofair/oh-my-claudecode)를 watch하세요
...
- **버그 보고**: [GitHub Issues](https://github.com/materialofair/oh-my-claudecode/issues)
